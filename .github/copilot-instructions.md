...
- **Internal API** (`api/v1/internal.py`): service-to-service endpoints called by FL client/server containers. No JWT auth — protected by Docker network isolation only. Includes `POST /internal/fl/progress`, `POST /internal/fl/round`, `POST /internal/predictions`. Do not add JWT guards to these.
- **WebSocket**: JWT cannot use `Depends()` in WS handlers — token is passed as query param `?token=<JWT>`. Connect via `ws://localhost:8000/api/v1/ws?token=<JWT>`. The `ws_manager` singleton (importable from `app.core.websocket`) handles all WS broadcast/send.
...
- **Tailwind v4**: uses `@tailwindcss/vite` plugin (not the PostCSS plugin). Do not add a `tailwind.config.js` or `postcss.config.js`.
...
- **VSS**: `fl_common/vss_utils.py` implements n-of-n XOR-based secret sharing for the CKKS secret key. Use `split_key()` / `reconstruct_and_get_context()` / `threshold_decrypt()` — do not manipulate TenSEAL private contexts directly.
- FL client mode is controlled by the `MODE` env var (`IDLE` | `MONITOR` | `TRAIN`).
- **CNN-LSTM model** (`fl_common/model.py`): input shape `(batch, seq_len=10, num_features=78)`, output is raw logit (apply sigmoid for probability). Only four layers get CKKS-encrypted: `lstm.weight_ih_l0`, `lstm.weight_hh_l0`, `fc.weight`, `fc.bias` (see `SELECTED_LAYERS`).
...
- Default admin: `admin` / `admin123` (seeded on first startup via `seed_admin()` in `main.py`).
- Docker container names follow the pattern `iot_ids_<service>` (e.g., `iot_ids_backend`, `iot_ids_fl_client_bank_a`); all services share the `iot_ids_network` Docker network.
- FL client container cert filenames are derived from `client_id` using title-case: `bank_a` → `Bank_A.crt` / `Bank_A.key` / `Bank_A_ed25519.pem`.

## GitNexus MCP

This codebase is indexed by GitNexus as **FYP_2026** (1519 symbols, 3342 relationships, 116 execution flows). Use it to navigate complex cross-service relationships:
